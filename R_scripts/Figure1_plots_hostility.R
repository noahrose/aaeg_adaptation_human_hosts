...
###############################################################
pdf(width=8,height=4,file='Supplementary Figures/Day_Effect.pdf',useDingbats=F)
...
popmeta$Region[popmeta $code%in%c('RAB','KBO')]<-'RAB'
popmeta $Region[popmeta $code%in%c('SHM','KWA')]<-'SHM'
popmeta $Region[popmeta $code%in%c('GND','ABK')]<-'ABK'
popmeta $Region[popmeta $code%in%c('NGO')]<-'NGO'
popmeta $Region[popmeta $code%in%c('THI')]<-'THI'

dat$Region<-popmeta$Region[match(dat$Population,popmeta$code)]
dat$Region[is.na(dat$Region)]<-as.character(dat$Population[is.na(dat$Region)])
dat$Region<-factor(dat$Region,levels=c('ORL','T51','NGO','THI','MIN','KED','OGD','OHI','KUM','KIN','AWK','LBV','LOP','FCV','ENT','WKE','RAB','SHM','ABK'))
gm<-glmmTMB(cbind(N_Hu,N_Alt)~Region+(1|Colony)+(1|Day),data=dat,family='betabinomial')
e2<-emmeans(gm,'Region')
conf2<-confint(e2,type='response')
plot(conf2)
pr<-aggregate(cbind(bio.bio15,bio.bio18,bio.bio13,bio.bio9,bio.bio16,dens20,Longitude,Latitude)~Region,data=popmeta,FUN=mean)
pr<-pr[match(as.character(conf2$Region)[-c(1,2)],pr$Region),]
pr$prob<-conf2$prob[-c(1:2)]
...
pdf(file='Supplementary Figures/Model_selection.pdf',width=4,height=4,useDingbats=F)
...
pdf(file='Supplementary Figures/Bioclim_selection.pdf',width=4,height=3,useDingbats=F)
...
pdf(file='Supplementary Figures/Bioclim18_selection.pdf',width=4,height=3,useDingbats=F)
par(mgp=c(1.2,.2,0),tck=-0.01,bty='l',mar=c(4,3,1,1))
...
#######################################################################################
# dens<-lm(logit(prob)~dens20,data=popmeta)
# bioresid<-resid(lm(logit(prob)~fitted(mp15)+bio.bio18,data=popmeta))+logit(mean(popmeta$prob))
# dens20resid<-resid(dens)+logit(mean(popmeta$prob))
# fitted(full18,type='model',terms=c('fitted(mp15)'))
...
pdf(file='-- FINAL subplots/Climate_pop_behavior.pdf',width=6,height=3,useDingbats=F)
...
pdf(file='-- FINAL subplots/bioclim_pop_behavior.pdf',width=2,height=3,useDingbats=F)
...
pdf(file='-- FINAL subplots/Climate_bioclim_pop_behavior_combined.pdf',width=8,height=2.5,useDingbats=F)
...
r<-raster('~/Downloads/wc2.0_10m_bio/wc2.0_bio_10m_15.tif')
pdf(width=6,height=6,file='-- FINAL subplots/Map.pdf',useDingbats=F)
...
pdf(width=6,height=6,file='-- FINAL subplots/Map2.pdf',useDingbats=F)
par(mar=c(1,1,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
################
# pdf(width=4,height=4,file='Supplementary Figures/HabitatMap.pdf')
# par(mar=c(3,3,1,1),tck=-0.01,mgp=c(1.2,.2,0))
# map('world',ylim=c(-40,40),xlim=c(-25,55),mar=c(3,3,1,1),col=gray(0.9),fill=T,border=gray(0.9))
# gradient.rect(-15,-17,5,-15,col=colorRampPalette(c('cornflowerblue','red'))(200))
# text(-5,-19,'Preference index')
...
# segments(psub$plotLong,psub$plotLat,psub$Longitude,psub$Latitude)
# points(psub$plotLong,psub$plotLat,cex=1.5,pch=20+psub$peri,bg=psub$prefcol,lwd=1)
# points(c(-10,0),c(-10,-10),cex=1.5,lwd=1,pch=c(0,1))
# text(c(-10,0),c(-13,-13),labels=c('Forest','Town'),cex=0.7)
# dev.off()

pdf(width=2,height=4,file='-- FINAL subplots/Habitat.pdf',useDingbats=F)
...
pdf(width=8,height=4,file='-- FINAL subplots/Prefs.pdf',useDingbats=F)
...
pdf(width=3,height=3,file='Supplementary Figures/Repeatability.pdf',useDingbats=F)
...
pdf(width=3,height=3,file='Supplementary Figures/Repeatability_grey.pdf',useDingbats=F)
par(mar=c(4,4,2,1),mgp=c(1.5,.2,0),tck=-0.01)
plot(fp[first],fp[second],xlab='Colony 1 preference index',ylab='Colony 2 preference index',bty='l',pch=21,bg='grey',axes=F,xlim=c(-1,1),ylim=c(-1,1),lwd=0.25)
text(-.6,.8,labels=expression(italic(R^2) == 0.60))
axis(1,at=c(-1,0,1),lwd=0.5)
axis(2,at=c(-1,0,1),lwd=0.5)
abline(0,1,lty=2)
...
pdf(width=3,height=3,file='Supplementary Figures/Responserate.pdf',useDingbats=F)
par(mar=c(4,4,2,1),mgp=c(1.5,.2,0),tck=-0.01)
...
pdf(width=12,height=4,file='Supplementary Figures/Responses.pdf',useDingbats=F)
par(mar=c(4,3,2,1),tck=0.01,mgp=c(1.2,.2,0))
coords=c(1,2,5,6,7,8,9,10,13,14,17,18,19,22,25,26,27,28,31,32,35,36,37,38,39,40,41,42,43)
...
pdf(width=4,height=4,file='Supplementary Figures/FigAltHost.pdf',useDingbats=F)

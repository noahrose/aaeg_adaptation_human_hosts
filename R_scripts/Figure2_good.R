...
########################################
aaa<-read.delim('admixture analyses/verily_unlinked_norepeat.3.Q',header=F,sep=' ')
sm$aaa<-NA
...
# sm$PopNoOutl[sm$Pop%in%c('SHM','BTT','PKT','OHI')&sm$aaa>0.01]<-NA
sm$fid<-0
...
pdf('-- FINAL subplots/admixture.pdf',width=8,height=3,useDingbats=F)
...
pdf('Supplementary Figures/admixture_k.pdf',width=8,height=8,useDingbats=F)
...
pm<-read.delim('../Figure1 stuff/colonies_conf.txt',stringsAsFactors=F)
...
pdf('Supplementary Figures/crossValidation.pdf',width=4,height=4,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01,bty='l')
err<-read.delim('admixture analyses/cv.out',sep=' ',header=F)[c(2:10,1),]
...
pdf('-- FINAL subplots/admixture_behavior_k6.pdf',width=3,height=3,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01,bty='l')
...
# pm$region<-NA
# pm$region[pm$Population%in%c('NGO','THI','MIN','KED','PKT','BTT','OGD','OHI','KUM','KIN','BOA','AWK')]<-'westaf'
# pm$region[pm$Population%in%c('LBV','LPV','FCV','ENT','VMB','KAK')]<-'centaf'
# pm$region[pm$Population%in%c('KBO','RAB','KWA','SHM','ABK','GND')]<-'eastaf'
# pm$rcol<-NA
# pm$rcol[pm$region=='westaf']<-'orange'
# pm$rcol[pm$region=='eastaf']<-'blue'
# pm$rcol[pm$region=='centaf']<-'lightblue'
# par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01,bty='l')
# plot(pm$q3.aaa,pm$pref,bg=pm$rcol,pch=21,ylim=c(-1,1),xlim=c(0,.5),xlab='Proportion Aaa',ylab='Preference index',axes=F,lwd=0.25)
# axis(1,lwd=0.5)
# axis(2,at=c(-1,0,1),lwd=0.5)
# text(.1,.8,labels=expression(italic(R^2) == 0.76))
# lines(seq(0.01,0.99,0.01),2*inv.logit(predict(lm.out,data.frame(q3.aaa=seq(.01,0.99,0.01))))-1,lty=2)
...
summary(lm.out2)
pdf('-- FINAL subplots/PCA.pdf',width=4,height=4,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01,bty='l')
...
pdf('-- FINAL subplots/PCA2.pdf',width=4,height=4,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01,bty='l')
...
plot(nSites,meanpis,lwd=0.1)

plot(thetas[['BKK']]$plotpos,rollmean(thetas[['BKK']]$Tajima,k=50,na.pad=T),cex=0.1,ylim=c(-2,2),ylab='Tajimas D',xlab='Position (Mb)',axes=F)
points(thetas[['NGO']]$plotpos,rollmean(thetas[['NGO']]$Tajima,k=50,na.pad=T),cex=0.1,col='purple')
points(thetas[['FCV']]$plotpos,rollmean(thetas[['FCV']]$Tajima,k=50,na.pad=T),cex=0.1,col='green')
points(thetas[['MIN']]$plotpos,rollmean(thetas[['MIN']]$Tajima,k=50,na.pad=T),cex=0.1,col='red')
...
axis(2)
abline(h=0,lty=2)
legend('topright',fill=c('black','purple','green','red'),legend=c('BKK','NGO','FCV','MIN'),ncol=4,bty='n')

ord<-c(levels(sm$Pop),'MASC')
pdf(file='Supplementary Figures/pi_pops.pdf',width=8,height=3,useDingbats=F)
...
pdf(file='Supplementary Figures/pi_genome.pdf',width=8,height=2,useDingbats=F)
...
# MIN.BKK<-fstread3('MIN.BKK',matchpos)
...
# minPBS<-getPBS(MIN.KED, BTT.MIN,BTT.KED,k=k)
...
pdf(file='Supplementary Figures/FST_pairwise.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
ngoPBSfine<-getPBS(BTT.NGO,FCV.NGO,BTT.FCV,k=1)
plot(BTT.BKK$plotpos,ngoPBSfine,cex=0.1,ylim=c(0,1),xlim=c(380e6+300e6,380e6+320e6),col='darkorange',axes=F, xlab='Position (Mb)',ylab='NGO PBS',type='l')
axis(2)
axis(1,at=seq(380e6+300e6,380e6+320e6,5e6),labels=seq(300e6,320e6,5e6))


pdf(file='-- FINAL subplots/PBS_pairwise.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf(file='-- FINAL subplots/PBS_pairwise_KED_OHI.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
plot(BTT.BKK$plotpos,ngoPBS,cex=0.1,ylim=c(0,.75),col='darkorange',axes=F, xlab='Position (Mb)',ylab='PBS',type='n')
rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col=grey(0.75))
...
pdf(file='-- FINAL subplots/PBS_pairwise_nobkk.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf(file='Supplementary Figures/PBS_pairwise_nobkk_BTT.SHM.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
plot(BTT.BKK$plotpos,ngoPBS,cex=0.1,ylim=c(0,.45),col='darkorange',axes=F, xlab='Position (Mb)',ylab='PBS',type='n')
rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col=grey(0.75))
...
pdf(file='Supplementary Figures/PBS_pairwise_nobkk_FCV.SHM.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
plot(BTT.BKK$plotpos,ngoPBS,cex=0.1,ylim=c(0,.45),col='darkorange',axes=F, xlab='Position (Mb)',ylab='PBS',type='n')
rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col=grey(0.75))
...
pdf(file='Supplementary Figures/FST_pairwise2.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf(file='Supplementary Figures/FST_pairwise2_MIN.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf(file='Supplementary Figures/FST_pairwise_MIN.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
# BKK.dxy<-dxyread('FCV.BKK',FCV.BKK)
# NGO.dxy<-dxyread('FCV.NGO',FCV.NGO)
# THI.dxy<-dxyread('FCV.THI',FCV.THI)
# OGD.dxy<-dxyread('FCV.OGD',FCV.OGD)

# BKK.dxy2<-dxyread('BTT.BKK',BTT.BKK)
# NGO.dxy2<-dxyread('BTT.NGO',BTT.NGO)
# THI.dxy2<-dxyread('BTT.THI',BTT.THI)
# OGD.dxy2<-dxyread('BTT.OGD',BTT.OGD)

# BTT.dxy<-dxyread('BTT.FCV',BTT.OGD)
# NGO.dxy3<-dxyread('BKK.NGO',BTT.NGO)
# THI.dxy3<-dxyread('BKK.THI',BTT.THI)
# OGD.dxy3<-dxyread('BKK.OGD',BTT.OGD)

pdf(file='Supplementary Figures/pi_pops.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf(file='Supplementary Figures/dxy_pairwise.pdf',width=8,height=2,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
# table(match(NGO.dxy$plotpos, thetas[[1]]$plotpos)==1:length(thetas[[1]]$plotpos))
...
# pdf(file='Supplementary Figures/dxy_pi.pdf',width=8,height=6,useDingbats=F)
# par(mfrow=c(3,4),mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01,bty='l',font.main=1)
# cols=rep('grey',nrow(BKK.dxy))
# outls<-BTT.NGO$outl&BTT.THI$outl&BTT.OGD$outl
# cols[outls&thetas[[1]][,2]=='NC_035107.1']<-'red'
# cols[outls&thetas[[1]][,2]=='NC_035108.1']<-'blue'

# ##########

# plot(allpis[,'BKK'],dxys[['BTT.BKK.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='BKK vs BTT',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'BKK'])[outls],(dxys[['BTT.BKK.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'BKK'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BTT.BKK.dxy']]$dxy,na.rm=T),lty=2)
# legend('topleft',fill=c('red','blue'),legend=c('Chr. 1','Chr. 2'),bty='n',cex=0.8)

# plot(allpis[,'NGO'],dxys[['BTT.NGO.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='NGO vs BTT',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'NGO'])[outls],(dxys[['BTT.NGO.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'NGO'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BTT.NGO.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'THI'],dxys[['BTT.THI.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='THI vs BTT',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'THI'])[outls],(dxys[['BTT.THI.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'THI'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BTT.THI.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'OGD'],dxys[['BTT.OGD.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='OGD vs BTT',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'OGD'])[outls],(dxys[['BTT.OGD.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'OGD'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BTT.OGD.dxy']]$dxy,na.rm=T),lty=2)

# ###############

# plot(allpis[,'BKK'],dxys[['FCV.BKK.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='BKK vs FCV',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'BKK'])[outls],(dxys[['FCV.BKK.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'BKK'],na.rm=T),lty=2)
# abline(h=mean(dxys[['FCV.BKK.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'NGO'],dxys[['FCV.NGO.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='NGO vs FCV',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'NGO'])[outls],(dxys[['FCV.NGO.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'NGO'],na.rm=T),lty=2)
# abline(h=mean(dxys[['FCV.NGO.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'THI'],dxys[['FCV.THI.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='THI vs FCV',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'THI'])[outls],(dxys[['FCV.THI.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'THI'],na.rm=T),lty=2)
# abline(h=mean(dxys[['FCV.THI.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'OGD'],dxys[['FCV.OGD.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='OGD vs FCV',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'OGD'])[outls],(dxys[['FCV.OGD.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'OGD'],na.rm=T),lty=2)
# abline(h=mean(dxys[['FCV.OGD.dxy']]$dxy,na.rm=T),lty=2)


# ##########

# plot(allpis[,'BTT'],dxys[['BTT.FCV.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='FCV vs BTT',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'BTT'])[outls],(dxys[['BTT.FCV.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'BTT'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BTT.FCV.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'NGO'],dxys[['BKK.NGO.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='NGO vs BKK',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'NGO'])[outls],(dxys[['BKK.NGO.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'NGO'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BKK.NGO.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'THI'],dxys[['BKK.THI.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='THI vs BKK',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'THI'])[outls],(dxys[['BKK.THI.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'THI'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BKK.THI.dxy']]$dxy,na.rm=T),lty=2)

# plot(allpis[,'OGD'],dxys[['BKK.OGD.dxy']]$dxy,cex=0.1,xlab=expression(pi),ylab=expression(d[xy]),main='OGD vs BKK',col=cols,ylim=c(0,0.05),xlim=c(0,0.05))
# points((allpis[,'OGD'])[outls],(dxys[['BKK.OGD.dxy']]$dxy)[outls],col=cols[outls],cex=0.3,pch=19)
# abline(v=mean(allpis[,'OGD'],na.rm=T),lty=2)
# abline(h=mean(dxys[['BKK.OGD.dxy']]$dxy,na.rm=T),lty=2)
...
load('pcadapt.Rdata')
sites<-fread('verily_maf05_africa.sites.gz')
...
# NGO.fd<-fdread('BKK.NGO.ENT',BTT.BKK)
# THI.fd<-fdread('BKK.THI.ENT',BTT.BKK)
# OGD.fd<-fdread('BKK.OGD.ENT',BTT.BKK)
# NGO.fd<-fdread('BKK.NGO.FCV',BTT.BKK)
# THI.fd<-fdread('BKK.THI.FCV',BTT.BKK)
# OGD.fd<-fdread('BKK.OGD.FCV',BTT.BKK)
# NGO.fd<-fdread('BKK.NGO.BTT',BTT.BKK)
# THI.fd<-fdread('BKK.THI.BTT',BTT.BKK)
# OGD.fd<-fdread('BKK.OGD.BTT',BTT.BKK)
...
# NGO.fd<-fdread('BTTna.NGO.BKK',BTT.BKK)
# THI.fd<-fdread('BTTna.THI.BKK',BTT.BKK)
# KED.fd<-fdread('BTTna.KED.BKK',BTT.BKK)
# PKT.fd<-fdread('BTTna.PKT.BKK',BTT.BKK)
# OGD.fd<-fdread('BTTna.OGD.BKK',BTT.BKK)
# OHI.fd<-fdread('BTTna.OHI.BKK',BTT.BKK)
# KUM.fd<-fdread('BTTna.KUM.BKK',BTT.BKK)
# KIN.fd<-fdread('BTTna.KIN.BKK',BTT.BKK)
# AWK.fd<-fdread('BTTna.AWK.BKK',BTT.BKK)
# LBV.fd<-fdread('LPV.LBV.BKK',BTT.BKK)
# FCV.fd<-fdread('LPV.FCV.BKK',BTT.BKK)
# KAK.fd<-fdread('VMB.KAK.BKK',BTT.BKK)
# ENT.fd<-fdread('VMB.ENT.BKK',BTT.BKK)
# KBO.fd<-fdread('SHMna.KBO.BKK',BTT.BKK)
# RAB.fd<-fdread('SHMna.RAB.BKK',BTT.BKK)
# KWA.fd<-fdread('SHMna.KWA.BKK',BTT.BKK)
# ABK.fd<-fdread('SHMna.ABK.BKK',BTT.BKK)
# GND.fd<-fdread('SHMna.GND.BKK',BTT.BKK)
...
# ABK.fd$fd,
# GND.fd$fd)
# colnames(fdmat)<-c('NGO','THI','MIN','KED','PKT','OGD','OHI','KUM','KIN','AWK','LBV','FCV','KAK','ENT','KBO','RAB','KWA','ABK','GND')

# library(boot)
# pmf<-pm[match(colnames(fdmat),pm$Population),]
# pref<-logit(pmf$prob)
# cc<-cor(t(fdmat),pref)**2
# cc2<-cor(t(fdmat[,1:10]),pref[1:10])**2
...
############################
library(plotrix)
# pdf(file='Supplementary Figures/Fd_pops.pdf',width=8,height=8,useDingbats=F)
# par(mfrow=c(4,1),mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
# plot(BTT.BKK3$plotpos,BKK.dxy$dxy,cex=0.1,ylim=c(0,1),col='darkblue',axes=F, xlab='Position (Mb)',ylab=expression(f[d]~BTT),type='n')
# rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
# rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col='pink')
# points(BTT.BKK3$plotpos,NGO.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='NGO'])
# points(BTT.BKK3$plotpos,THI.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='THI'])
# points(BTT.BKK3$plotpos,OGD.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='OGD'])
# points(BTT.BKK3$plotpos,KUM.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='KUM'])
# points(BTT.BKK3$plotpos,KIN.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='KIN'])
# points(BTT.BKK3$plotpos,AWK.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='AWK'])
# points(BTT.BKK3$plotpos,MIN.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='MIN'])
# points(BTT.BKK3$plotpos,PKT.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='PKT'])
# points(BTT.BKK3$plotpos,KED.fd$fd,cex=0.1,col=pm$prefcol[pm$Population=='KED'])
# gradient.rect(1.2e9,0.75,1.3e9,0.85,col=colorRampPalette(c('cornflowerblue','red'))(200))
# text(c(1.2e9,1.25e9,1.3e9),rep(0.95),c(-1,0,1))
...
# plot(BTT.BKK3$plotpos,BKK.dxy$dxy,cex=0.1,ylim=c(0,1),col='darkblue',axes=F, xlab='Position (Mb)',ylab=expression(f[d]~LPV),type='n')
# rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
# rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col='pink')
# points(BTT.BKK3$plotpos,LBV.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='LBV'])
# points(BTT.BKK3$plotpos,FCV.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='FCV'])
# axis(1,at=seq(0,300000000,100000000),labels=seq(0,300,100))
# axis(1,at=seq(380000000,780000000,100000000),labels=seq(0,400,100))
# axis(1,at=seq(920000000,1320000000,100000000),labels=seq(0,400,100))
# axis(2,at=seq(0,0.6,0.3))

# plot(BTT.BKK3$plotpos,BKK.dxy$dxy,cex=0.1,ylim=c(0,1),col='darkblue',axes=F, xlab='Position (Mb)',ylab=expression(f[d]~VMB),type='n')
# rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
# rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col='pink')
# points(BTT.BKK3$plotpos,KAK.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='KAK'])
# points(BTT.BKK3$plotpos,ENT.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='ENT'])
# axis(1,at=seq(0,300000000,100000000),labels=seq(0,300,100))
# axis(1,at=seq(380000000,780000000,100000000),labels=seq(0,400,100))
# axis(1,at=seq(920000000,1320000000,100000000),labels=seq(0,400,100))
# axis(2,at=seq(0,0.6,0.3))

# plot(BTT.BKK3$plotpos,BKK.dxy$dxy,cex=0.1,ylim=c(0,1),col='darkblue',axes=F, xlab='Position (Mb)',ylab=expression(f[d]~SHM),type='n')
# rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
# rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col='pink')
# points(BTT.BKK3$plotpos,KBO.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='KBO'])
# points(BTT.BKK3$plotpos,RAB.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='RAB'])
# points(BTT.BKK3$plotpos,KWA.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='KWA'])
# points(BTT.BKK3$plotpos,AWK.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='ABK'])
# points(BTT.BKK3$plotpos,GND.fd$fd,cex=0.1,col= pm$prefcol[pm$Population=='GND'])
# axis(1,at=seq(0,300000000,100000000),labels=seq(0,300,100))
# axis(1,at=seq(380000000,780000000,100000000),labels=seq(0,400,100))
# axis(1,at=seq(920000000,1320000000,100000000),labels=seq(0,400,100))
# axis(2,at=seq(0,0.6,0.3))
...
sc<-read.csv('128_Threshold_Characters_pop.csv',stringsAsFactors=F)
...
pdf('-- FINAL subplots/scaling_behavior_k6.pdf',width=3,height=3,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf('-- FINAL subplots/scaling_behavior_k6_noaaa.pdf',width=3,height=3,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
...
pdf('-- FINAL subplots/scaling_behavior_k6_aaa_heldout.pdf',width=3,height=3,useDingbats=F)
par(mar=c(3,3,1,1),mgp=c(1.5,.5,0),tck=-0.01)
plot(pmg$percArea,pmg$pref,bg=pmg$k6col,pch=21,ylim=c(-1,1),xlab='Percent white scales',ylab='Preference index',axes=F,lwd=0.25,cex=2)
axis(1,at=c(.2,.4,.6),labels=c(20,40,60),lwd=0.5)
axis(2,at=c(-1,0,1),lwd=0.5)
...
pdf(width=8,height=1.5,file='-- FINAL subplots/Scaling.pdf',useDingbats=F)
...
fdread2<-function(four){
	curr<-read.delim(paste('FST/',four,'.informative.fd',sep=''),header=F)
	curr$plotpos<-curr[,2]+50000
	curr$plotpos[curr[,1]=='NC_035108.1']=curr$plotpos[curr[,1]=='NC_035108.1']+380000000
	curr$plotpos[curr[,1]=='NC_035109.1']=curr$plotpos[curr[,1]=='NC_035109.1']+920000000
	curr$fd<-curr[,3]/curr[,5]
	curr$fd[curr$fd<0]<-0
	return(curr)
}

pdf(file='Supplementary Figures/Fd_informative_sites.pdf',width=8,height=6,useDingbats=F)
par(mfrow=c(3,1),mar=c(1,3,1,1),oma=c(3,0,0,0),mgp=c(1.5,.5,0),tck=-0.01)
NGO.fd2<-fdread2('BTT.NGO.BKK.Outgroup')
plot(NGO.fd2$plotpos,NGO.fd2$fd,type='n',axes=F,xlab='',ylab='BTT,NGO;BKK,MASC',ylim=c(0,0.6))
rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col=grey(0.75))
lines(NGO.fd2$plotpos[NGO.fd2$V1=='NC_035107.1'],NGO.fd2$fd[NGO.fd2$V1=='NC_035107.1'],col='darkorange',lwd=2)
lines(NGO.fd2$plotpos[NGO.fd2$V1=='NC_035108.1'],NGO.fd2$fd[NGO.fd2$V1=='NC_035108.1'],col='darkorange',lwd=2)
lines(NGO.fd2$plotpos[NGO.fd2$V1=='NC_035109.1'],NGO.fd2$fd[NGO.fd2$V1=='NC_035109.1'],col='darkorange',lwd=2)
axis(2)
THI.fd2<-fdread2('BTT.THI.BKK.Outgroup')
plot(NGO.fd2$plotpos,NGO.fd2$fd,type='n',axes=F,xlab='',ylab='BTT,THI;BKK,X',ylim=c(0,0.6))
rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col=grey(0.75))
lines(THI.fd2$plotpos[THI.fd2$V1=='NC_035107.1'],THI.fd2$fd[THI.fd2$V1=='NC_035107.1'],col='darkcyan',lwd=2)
lines(THI.fd2$plotpos[THI.fd2$V1=='NC_035108.1'],THI.fd2$fd[THI.fd2$V1=='NC_035108.1'],col='darkcyan',lwd=2)
lines(THI.fd2$plotpos[THI.fd2$V1=='NC_035109.1'],THI.fd2$fd[THI.fd2$V1=='NC_035109.1'],col='darkcyan',lwd=2)
axis(2)
THI.fd3<-fdread2('BTT.THI.BKK.FCV')
lines(THI.fd3$plotpos[THI.fd3$V1=='NC_035107.1'],THI.fd3$fd[THI.fd3$V1=='NC_035107.1'],col='cyan',lwd=2)
lines(THI.fd3$plotpos[THI.fd3$V1=='NC_035108.1'],THI.fd3$fd[THI.fd3$V1=='NC_035108.1'],col='cyan',lwd=2)
lines(THI.fd3$plotpos[THI.fd3$V1=='NC_035109.1'],THI.fd3$fd[THI.fd3$V1=='NC_035109.1'],col='cyan',lwd=2)
axis(2)
legend('topright',fill=c('darkcyan','cyan'),legend=c('MASC','FCV'),bty='n')
# THI.fd4<-fdread2('BTT.THI.NGO.FCV')
# plot(NGO.fd2$plotpos,NGO.fd2$fd,type='n',axes=F,xlab='',ylab='',ylim=c(0,0.8))
# rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
# rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col='pink')
# lines(THI.fd4$plotpos[THI.fd4$V1=='NC_035107.1'],THI.fd4$fd[THI.fd4$V1=='NC_035107.1'],col='darkcyan')
# lines(THI.fd4$plotpos[THI.fd4$V1=='NC_035108.1'],THI.fd4$fd[THI.fd4$V1=='NC_035108.1'],col='darkcyan')
# lines(THI.fd4$plotpos[THI.fd4$V1=='NC_035109.1'],THI.fd4$fd[THI.fd4$V1=='NC_035109.1'],col='darkcyan')
# axis(2)
OGD.fd2<-fdread2('BTT.OGD.BKK.Outgroup')
plot(NGO.fd2$plotpos,NGO.fd2$fd,type='n',axes=F,xlab='',ylab='BTT,OGD;BKK,MASC',ylim=c(0,0.6))
rect(rects2[,1],rects2[,2],rects2[,3],rects2[,4],border=F,col=grey(0.9))
rect(rects3[,1],rects3[,2],rects3[,3],rects3[,4],border=F,col=grey(0.75))
lines(OGD.fd2$plotpos[OGD.fd2$V1=='NC_035107.1'],OGD.fd2$fd[OGD.fd2$V1=='NC_035107.1'],col='darkmagenta',lwd=2)
lines(OGD.fd2$plotpos[OGD.fd2$V1=='NC_035108.1'],OGD.fd2$fd[OGD.fd2$V1=='NC_035108.1'],col='darkmagenta',lwd=2)
lines(OGD.fd2$plotpos[OGD.fd2$V1=='NC_035109.1'],OGD.fd2$fd[OGD.fd2$V1=='NC_035109.1'],col='darkmagenta',lwd=2)
axis(2)
axis(1,at=seq(0,300000000,100000000),labels=seq(0,300,100))
axis(1,at=seq(380000000,780000000,100000000),labels=seq(0,400,100))
axis(1,at=seq(920000000,1320000000,100000000),labels=seq(0,400,100))
...
table(NGO.fd$fd[indseq]>quantile(NGO.fd$fd[indseq],probs=0.9,na.rm=T),outls2[indseq])
table(THI.fd$fd[indseq]>quantile(THI.fd$fd[indseq],probs=0.9,na.rm=T),outls2[indseq])
table(OGD.fd$fd[indseq]>quantile(OGD.fd$fd[indseq],probs=0.9,na.rm=T),outls2[indseq])

cor.test(NGO.fd$fd[indseq],ngoPBS[indseq],method='spearman')
cor.test(THI.fd$fd[indseq],thiPBS[indseq],method='spearman')
cor.test(OGD.fd$fd[indseq],ogdPBS[indseq],method='spearman')
cor.test(MIN.fd$fd[indseq],minPBS[indseq],method='spearman')

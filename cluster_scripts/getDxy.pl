#!/usr/bin/perl
#NHR modification of the script distributed with ngsTools
...
#
#NHR November 2019: Edited to account for mafs missing in either file, assumes both mafs are sorted
#
...
while($line1=<POP1MAF>){
	...
	$line2=<POP2MAF>;
	if(!$line2){print STDERR "Reached end of file 2 before file 1, terminating\n";last;}
        chomp $line2;my @parts2=split('\t',$line2);

        while(($parts[0] cmp $parts2[0]) < 0){
                print STDERR "Mismatch:$parts[0]\t$parts[1]\t$parts2[0]\t$parts2[1]\n";
                $line1=<POP1MAF>;
                if($line1){
                        chomp $line1;@parts=split('\t',$line1);
                } else {last;}
        }
        ...
        while(($parts[0] cmp $parts2[0]) > 0){
                ...
                $line2=<POP2MAF>;
                if($line2){
                        chomp $line2;@parts2=split('\t',$line2);
                } else {last;}
        }


	while($parts[1]<$parts2[1]){
		print STDERR "Mismatch:$parts[0]\t$parts[1]\t$parts2[0]\t$parts2[1]\n";
		$line1=<POP1MAF>;
		if($line1){
	        	chomp $line1;@parts=split('\t',$line1);
